    ...
    <div class="flex flex-col gap-2">
        <div class="flex items-center gap-2">
            ...
            <button v-if="modelValue?.id" @click="handleEdit"
                class="p-1 text-gray-500 hover:text-blue-600 transition-colors" title="Editar país seleccionado">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                    stroke="currentColor" class="w-5 h-5">
            ...
            <button v-if="modelValue?.id" @click="handleDelete"
                class="p-1 text-gray-500 hover:text-red-600 transition-colors" title="Eliminar país seleccionado">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                    stroke="currentColor" class="w-5 h-5">
                    <path stroke-linecap="round" stroke-linejoin="round"
        ...
        <div class="flex gap-2">
            <USelectMenu v-model="modelValue" option-attribute="concat" :options="options" :searchable="true"
                v-model:query="query" :clearSearchOnClose="true" class="min-w-[200px] w-full">
            </USelectMenu>
        </div>
    </div>
</template>
...
const showEditorModal = async (isEdit: boolean = false) => {
    ...
    const { value: formValues } = await Swal.fire({
        title: `<span class="text-lg font-semibold text-gray-800">${isEdit ? 'Editar' : 'Nuevo'} País</span>`,
        html: `
      <div class="space-y-4">
        <div>
          <label class="block text-sm text-gray-600 mb-1">Código (opcional)</label>
          <input id="code" 
        ...
        <div>
          <label class="block text-sm text-gray-600 mb-1">Nombre del País*</label>
          <input id="name" 
                 class="w-full px-3 py-2 text-sm border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500
    ...
    `,
        ...
        preConfirm: () => {
            ...
            let error = ''
            if (!name || name.length < 2) error += 'El nombre es requerido (mínimo 2 caracteres)<br>'
            if (code && code.length > 10) error += 'El código no puede exceder 10 caracteres'
        ...
        didOpen: () => {
            ...
            const validateInputs = () => {
                const nameValid = inputs[1].value.trim().length >= 2
                const codeValid = inputs[0].value.trim().length <= 10
                confirmButton.disabled = !(nameValid && codeValid)
            }
    ...
    if (formValues) {
        try {
            ...
            Swal.fire({
                icon: 'success',
                title: '¡Actualizado!',
                text: 'El país se editó correctamente',
                confirmButtonColor: '#3b82f6',
                customClass: {
                    popup: 'rounded-xl',
                    confirmButton: 'px-4 py-2 text-sm font-medium'
...
const showErrorAlert = (action: string) => {
    Swal.fire({
        icon: 'error',
        title: 'Error',
        text: `No se pudo ${action} el país`,
        confirmButtonColor: '#3b82f6',
        customClass: {
            popup: 'rounded-xl',
            confirmButton: 'px-4 py-2 text-sm font-medium'
...
const retrieveFromApi = async () => {
    try {
        const response = await $fetch<any>("api/country/", {
            query: { search: query.value }
        });
        options.value = response.results.map((country: any) => ({
            ...country,
