...
<template>
    <div class="flex flex-col gap-2">
      ...
      <UModal v-model="isModalOpen">
        <div class="p-4 bg-white rounded-lg space-y-4">
          <h3 class="text-lg font-semibold">{{ isEditMode ? 'Editar' : 'Nuevo' }} Sub Tipo de Archivo</h3>
          
          <div>
            <label class="block text-sm text-gray-600 mb-1">Nombre</label>
            <UInput 
              v-model="formData.name" 
              placeholder="Nombre del tipo de archivo"
              class="w-full"
            />
          </div>
  ...
  const props = defineProps({
    modelValue: {
      type: [Object, Number] as () => FileType | number,
      default: () => ({})
    },
    labelpersonal: {
      type: String,
      default: 'Tipo de Archivo',
    }
  });
  
  const options = ref<FileType[]>([]);
  const query = ref("");
  const modelValue = defineModel<any>({ default: null });
  const isModalOpen = ref(false);
  const isEditMode = ref(false);
  
  const formData = ref<FileType>({
    name: ''
  });
  
  // Sincronizar valor inicial con opciones
  const syncInitialValue = () => {
    if (typeof modelValue.value === 'object' && modelValue.value?.id) {
      const selected = options.value.find(opt => opt.id === modelValue.value.id);
      if (selected) modelValue.value = selected;
    } else if (typeof modelValue.value === 'number') {
      const selected = options.value.find(opt => opt.id === modelValue.value);
      if (selected) modelValue.value = selected;
    }
  };
  ...
  // Watcher para sincronizar valor con opciones
  watch(options, () => {
    syncInitialValue();
  });
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const url = formData.value.id ? `/api/filessubtypes/${formData.value.id}/` : '/api/filessubtypes/';
      ...
      if (data.value) {
        ...
        Swal.fire({
          icon: 'success',
          title: '¡Éxito!',
          text: `Tipo de archivo ${isEditMode.value ? 'actualizado' : 'creado'} correctamente`,
          confirmButtonColor: '#3b82f6'
        });
      }
    } catch (error) {
      console.error("Error guardando tipo de archivo:", error);
  ...
  const handleDelete = async () => {
    ...
    if (result.isConfirmed) {
      try {
        await useFetch(`/api/filessubtypes/${modelValue.value.id}/`, {
          method: 'DELETE'
        });
        ...
        console.error("Error eliminando tipo de archivo:", error);
  ...
  const retrieveFromApi = async () => {
    try {
      const response = await $fetch<{ results: FileType[] }>("/api/filessubtypes/", {
        query: { 
          search: query.value,
        }
      });
      ...
      console.error("Error cargando tipos de archivo:", error);
      showErrorAlert('cargar tipos de archivo');
    }
  };
