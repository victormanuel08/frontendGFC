...
<template>
    <div class="flex flex-col gap-2">
      ...
      <UModal v-model="isModalOpen">
        <div class="p-4 bg-white rounded-lg space-y-4">
          <h3 class="text-lg font-semibold">{{ isEditMode ? 'Editar' : 'Nuevo' }} Tipo de Archivo</h3>
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const url = formData.value.id ? `/api/filestypes/${formData.value.id}/` : '/api/filestypes/';
  ...
  const handleDelete = async () => {
    ...
    if (result.isConfirmed) {
      try {
        await useFetch(`/api/filestypes/${modelValue.value.id}/`, {
          method: 'DELETE'
        });
  ...
  const retrieveFromApi = async () => {
    try {
      const response = await $fetch<{ results: FileType[] }>("/api/filestypes/", {
        query: { 
          search: query.value,
        }
      });
