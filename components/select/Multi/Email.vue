<template>
    <div class="flex flex-col gap-2">
      <div class="flex items-center gap-2">
        <label class="block">Correos Electrónicos:</label>
        <!-- Botones CRUD -->
        <button 
          @click="handleAdd" 
          class="p-1 text-gray-500 hover:text-green-600 transition-colors"
          title="Agregar nuevo correo"
        >
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
            <path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
          </svg>
        </button>
  
        <button 
          v-if="selectedEmails.length === 1" 
          @click="handleEdit"
          class="p-1 text-gray-500 hover:text-blue-600 transition-colors" 
          title="Editar correo seleccionado"
        >
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
            <path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10" />
          </svg>
        </button>
  
        <button 
          v-if="selectedEmails.length === 1" 
          @click="handleDelete"
          class="p-1 text-gray-500 hover:text-red-600 transition-colors" 
          title="Eliminar correo seleccionado"
        >
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
            <path stroke-linecap="round" stroke-linejoin="round" d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0" />
          </svg>
        </button>
      </div>
  
      <!-- Burbujas de selección numeradas -->
      <div v-if="selectedEmails.length > 0" class="flex flex-wrap gap-2 p-2 border rounded-lg min-h-12">
        <UTooltip 
          v-for="(email, index) in selectedEmails" 
          :key="email.id"
          :text="email.email"
          :popper="{ placement: 'top' }"
        >
          <span class="inline-flex items-center px-2 py-1 rounded-full bg-blue-100 text-blue-800 text-xs font-medium">
            {{ index + 1 }}
            <button 
              @click.stop="removeEmail(email)"
              class="ml-1 text-blue-500 hover:text-blue-700 text-xs"
            >
              &times;
            </button>
          </span>
        </UTooltip>
      </div>
  
      <!-- Selector múltiple -->
      <div class="flex gap-2">
        <USelectMenu 
          v-model="selectedEmails" 
          option-attribute="email" 
          :options="filteredOptions"
          :searchable="true"
          multiple
          v-model:query="query" 
          :clearSearchOnClose="true" 
          class="flex-1"
          placeholder="Buscar o seleccionar correos..."
        >
          <template #label>
            <span v-if="selectedEmails.length === 0">Seleccionar correos...</span>
          </template>
        </USelectMenu>
      </div>
  
      <!-- Modal de Edición/Creación -->
      <UModal v-model="isModalOpen">
        <div class="p-4 bg-white rounded-lg space-y-4">
          <h3 class="text-lg font-semibold">{{ isEditMode ? 'Editar' : 'Nuevo' }} Correo Electrónico</h3>
          
          <div>
            <label class="block text-sm text-gray-600 mb-1">Email</label>
            <UInput 
              v-model="formData.email" 
              type="email"
              placeholder="Ej: contacto@empresa.com"
              class="w-full"
            />
          </div>
  
          <div>
            <label class="block text-sm text-gray-600 mb-1">Contraseña</label>
            <UInput 
              v-model="formData.password" 
              type="password"
              placeholder="••••••••"
              class="w-full"
            />
          </div>
  
          <div class="flex justify-end gap-2">
            <UButton color="gray" @click="isModalOpen = false">Cancelar</UButton>
            <UButton color="primary" @click="handleSubmit">{{ isEditMode ? 'Guardar' : 'Crear' }}</UButton>
          </div>
        </div>
      </UModal>
    </div>
  </template>
  
  <script setup lang="ts">
  import { ref, computed, watch, onMounted } from 'vue'
  import Swal from 'sweetalert2'
  
  interface Email {
    id?: number
    email: string
    password: string
  }
  
  // Modelo para los correos seleccionados
  const selectedEmails = defineModel<any[]>({ 
    default: () => [],
    required: true 
  })
  
  const options = ref<Email[]>([])
  const query = ref('')
  const isModalOpen = ref(false)
  const isEditMode = ref(false)
  
  const formData = ref<Email>({
    email: '',
    password: ''
  })
  
  // Filtrar opciones para no mostrar las ya seleccionadas
  const filteredOptions = computed(() => {
    const selectedIds = selectedEmails.value.map(e => e.id)
    return options.value.filter(option => !selectedIds.includes(option.id))
  })
  
  // Carga inicial de datos
  onMounted(async () => {
    await retrieveFromApi()
  })
  
  const handleAdd = () => {
    isEditMode.value = false
    formData.value = { email: '', password: '' }
    isModalOpen.value = true
  }
  
  const handleEdit = () => {
    if (selectedEmails.value.length !== 1) return
    
    isEditMode.value = true
    formData.value = {
      id: selectedEmails.value[0].id,
      email: selectedEmails.value[0].email,
      password: selectedEmails.value[0].password
    }
    isModalOpen.value = true
  }
  
  const removeEmail = (email: Email) => {
    selectedEmails.value = selectedEmails.value.filter(e => e.id !== email.id)
  }
  
  const handleSubmit = async () => {
    // Validación
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(formData.value.email)) {
      Swal.fire({
        icon: 'error',
        title: 'Error',
        text: 'Por favor ingrese un email válido',
        confirmButtonColor: '#3b82f6'
      })
      return
    }
  
    if (formData.value.password.length < 6) {
      Swal.fire({
        icon: 'error',
        title: 'Error',
        text: 'La contraseña debe tener al menos 6 caracteres',
        confirmButtonColor: '#3b82f6'
      })
      return
    }
  
    try {
      const url = formData.value.id 
        ? `/api/email/${formData.value.id}/` 
        : '/api/email/'
      
      const method = formData.value.id ? 'PUT' : 'POST'
  
      const { data } = await useFetch(url, {
        method,
        body: formData.value
      })
  
      if (data.value) {
        await retrieveFromApi()
        isModalOpen.value = false
        
        Swal.fire({
          icon: 'success',
          title: '¡Éxito!',
          text: `Correo electrónico ${isEditMode.value ? 'actualizado' : 'creado'} correctamente`,
          confirmButtonColor: '#3b82f6'
        })
      }
    } catch (error) {
      console.error('Error guardando correo:', error)
      showErrorAlert('guardar cambios')
    }
  }
  
  const handleDelete = async () => {
    if (selectedEmails.value.length !== 1) return
  
    const result = await Swal.fire({
      title: '¿Eliminar correo electrónico?',
      text: `Estás por eliminar: ${selectedEmails.value[0].email}`,
      icon: 'warning',
      showCancelButton: true,
      confirmButtonColor: '#d33',
      cancelButtonColor: '#3085d6',
      confirmButtonText: 'Sí, eliminar!'
    })
  
    if (result.isConfirmed) {
      try {
        await useFetch(`/api/email/${selectedEmails.value[0].id}/`, {
          method: 'DELETE'
        })
        
        await retrieveFromApi()
        selectedEmails.value = selectedEmails.value.filter(e => e.id !== selectedEmails.value[0].id)
        
        Swal.fire('¡Eliminado!', 'El correo electrónico fue eliminado', 'success')
      } catch (error) {
        console.error('Error eliminando correo:', error)
        showErrorAlert('eliminar')
      }
    }
  }
  
  const retrieveFromApi = async () => {
    try {
      const response = await $fetch<any>('api/email/', {
        query: { search: query.value }
      })
      options.value = response.results
    } catch (error) {
      console.error('Error cargando correos:', error)
      showErrorAlert('cargar correos electrónicos')
    }
  }
  
  const showErrorAlert = (action: string) => {
    Swal.fire({
      icon: 'error',
      title: 'Error',
      html: `<div class="text-sm">No se pudo ${action}</div>`,
      confirmButtonColor: '#3b82f6',
      customClass: {
        popup: 'rounded-xl',
        confirmButton: 'px-4 py-2 text-sm font-medium'
      }
    })
  }
  
  watch(query, retrieveFromApi)
  </script>