...
const showEditorModal = async (isEdit: boolean = false) => {
    ...
    const { value: formValues } = await Swal.fire({
        title: `<span class="text-lg font-semibold text-gray-800">${isEdit ? 'Editar' : 'Nuevo'} Tipo de Tercero</span>`,
        html: `
      <div class="space-y-4">
        <div>
    ...
    `,
        ...
        preConfirm: () => {
            ...
            if (name.length > 100) error += 'El nombre no puede exceder los 100 caracteres'
        ...
        didOpen: () => {
            ...
            const validateInput = () => {
                const nameValid = input.value.trim().length >= 3 && input.value.trim().length <= 100
...
const handleAdd = async () => {
    ...
    if (formValues) {
        try {
            const newItem = await $fetch('api/third_types/', {
                method: 'POST',
                body: {
            ...
            Swal.fire({
                icon: 'success',
                title: '¡Creado!',
                text: 'El tipo de tercero se creó correctamente',
                confirmButtonColor: '#3b82f6',
                customClass: {
                    popup: 'rounded-xl',
                    confirmButton: 'px-4 py-2 text-sm font-medium'
...
const handleEdit = async () => {
    ...
    if (formValues) {
        try {
            const updatedItem = await $fetch(`api/third_types/${modelValue.value.id}/`, {
                method: 'PUT',
                body: {
                    name: formValues.name
                }
            ...
            Swal.fire({
                icon: 'success',
                title: '¡Actualizado!',
                text: 'El tipo de tercero se editó correctamente',
                confirmButtonColor: '#3b82f6',
                customClass: {
                    popup: 'rounded-xl',
                    confirmButton: 'px-4 py-2 text-sm font-medium'
...
const handleDelete = async () => {
    ...
    if (result.isConfirmed) {
        try {
            await $fetch(`api/third_types/${modelValue.value.id}/`, {
                method: 'DELETE'
            });
            ...
            Swal.fire('Eliminado!', 'El tipo de tercero fue eliminado', 'success');
        } catch (error) {
...
const showErrorAlert = (action: string) => {
    Swal.fire({
        icon: 'error',
        title: 'Error',
        text: `No se pudo ${action} el tipo de tercero`,
        confirmButtonColor: '#3b82f6',
        customClass: {
            popup: 'rounded-xl',
            confirmButton: 'px-4 py-2 text-sm font-medium'
...
const retrieveFromApi = async () => {
    try {
        const response = await $fetch<any>("api/third_types/", {
