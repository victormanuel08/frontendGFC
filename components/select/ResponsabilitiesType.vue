<template>
    <div class="flex flex-col gap-2">
        <div class="flex items-center gap-2">
            <label class="block">Tipo de Responsabilidad:</label>
            <!-- Icono Agregar -->
...
const showEditorModal = async (isEdit: boolean = false) => {
    ...
    const { value: formValues } = await Swal.fire({
        title: `<span class="text-lg font-semibold text-gray-800">${isEdit ? 'Editar' : 'Nuevo'} Tipo de Responsabilidad</span>`,
        html: `
      <div class="space-y-4">
        <div>
...
const handleAdd = async () => {
    ...
    if (formValues) {
        try {
            const newItem = await $fetch('api/responsabilitys_types/', {
                method: 'POST',
                body: {
...
const handleEdit = async () => {
    ...
    if (formValues) {
        try {
            const updatedItem = await $fetch(`api/responsabilitys_types/${modelValue.value.id}/`, {
                method: 'PUT',
                body: {
                    code: formValues.code,
                    name: formValues.name
...
const handleDelete = async () => {
    ...
    if (result.isConfirmed) {
        try {
            await $fetch(`api/responsabilitys_types/${modelValue.value.id}/`, {
                method: 'DELETE'
            });
...
const retrieveFromApi = async () => {
    try {
        const response = await $fetch<any>("api/responsabilitys_types/", {
